...
ConfigURLHandler::ConfigURLHandler() : BaseURLHandler("text/plain"),
...
void ConfigURLHandler::handleRequest( struct mg_connection *conn,
        const struct mg_request_info *ri, void *data)
{
    std::list<std::string> params = BaseURLHandler::splitQueryString(ri->query_string);
    CogServer& cogserver = static_cast<CogServer&>(server());
    Request* request = cogserver.createRequest("json-get-config");
    if (request == NULL) {
...
void ConfigURLHandler::OnRequestComplete()
{
